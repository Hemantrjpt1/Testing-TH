...
local TSManualGroup = Tabs.TSQuestTab:AddLeftGroupbox("Manual TP", "navigation")
local TSWatchTowerGroup = Tabs.TSQuestTab:AddRightGroupbox("Watch Towers (Outskirts)", "shield")
...
-- ==========================================
-- RETRY FAILED ALT (Stuck Reward Screen Fix)
-- ==========================================

getgenv().RetryFailedAlt = false
getgenv().RetryFailedTimeout = 10

MainGroup:AddToggle("RetryFailedAltToggle", {
    Text = "Retry Failed Alt",
    Default = false,
    Tooltip = "If stuck on reward screen for X seconds, spam return to lobby"
})
Toggles.RetryFailedAltToggle:OnChanged(function()
    getgenv().RetryFailedAlt = Toggles.RetryFailedAltToggle.Value
    
    if getgenv().RetryFailedAlt then
        task.spawn(function()
            local rewardStuckTime = 0
            
            while getgenv().RetryFailedAlt do
                task.wait(0.5)
                ...
                pcall(function()
                    ...
                    if rewardsGui and rewardsGui.Visible then
                        rewardStuckTime = rewardStuckTime + 0.5
                        
                        -- Check if stuck longer than timeout
                        if rewardStuckTime >= getgenv().RetryFailedTimeout then
                            Library:Notify({
                                Title = "Retry Failed Alt",
                                Description = "Stuck for " .. rewardStuckTime .. "s! Spamming lobby return...",
                                Time = 5
                            })
                            ...
                            -- Spam return to lobby 5-8 times
                            local spamCount = math.random(5, 8)
                            ...
                            for i = 1, spamCount do
                                ...
                                pcall(function()
                                    postRemote:FireServer("Functions", "Teleport", "Lobby")
                                end)
                                
                                Library:Notify({
                                    Title = "Lobby Call " .. i .. "/" .. spamCount,
                        ...
                        -- Reset timer when reward screen closes
                        rewardStuckTime = 0
                    end
                end)
            end
...
-- Timeout Slider
MainGroup:AddSlider("RetryFailedTimeoutSlider", {
    Text = "Stuck Timeout (seconds)",
    Default = 10,
    Min = 5,
    Max = 30,
    Rounding = 0,
    Tooltip = "How long to wait before spamming lobby return"
})
Options.RetryFailedTimeoutSlider:OnChanged(function()
    getgenv().RetryFailedTimeout = Options.RetryFailedTimeoutSlider.Value
end)
...
-- ==========================================
-- AUTO COLLECT SKILLS CRATE (Fixed Position)
-- ==========================================

getgenv().AutoCollectCrate = false

WavesFarmGroup:AddToggle("AutoCollectCrateToggle", {
    Text = "Auto Collect Skills Crate",
    Default = false,
    Tooltip = "Auto Collect Skills Crates in Waves"
})
Toggles.AutoCollectCrateToggle:OnChanged(function()
    getgenv().AutoCollectCrate = Toggles.AutoCollectCrateToggle.Value
    
    if getgenv().AutoCollectCrate then
        task.spawn(function()
            while getgenv().AutoCollectCrate do
                pcall(function()
                    local myCrate = workspace.Waves and workspace.Waves.Crates and workspace.Waves.Crates:FindFirstChild("Crate_" .. lp.Name)
                    
                    if myCrate then
                        Library:Notify({ 
                            Title = "📦 Crate Found!", 
                            Description = "Pausing farm to collect...", 
                            Time = 3 
                        })
                        ...
                        -- Fixed position at crate
                        local root = lp.Character and lp.Character:FindFirstChild("HumanoidRootPart")
                        if root then
                            local cratePos = myCrate:GetPivot().Position
                            root.CFrame = CFrame.new(cratePos + Vector3.new(0, 5, 0))
                            root.AssemblyLinearVelocity = Vector3.zero -- Stop movement
                            root.AssemblyAngularVelocity = Vector3.zero
                            Library:Notify({ Title = "📦", Description = "At crate, collecting...", Time = 1.5 })
                        end
                        
                        -- Stay at crate for 3 seconds
                        task.wait(2)
                        
                        -- TP to Skills Zone (fixed position)
                        local skillsZone = workspace.Waves.Zones:FindFirstChild("Skills")
                        if skillsZone and root then
                            local zonePos = skillsZone:GetPivot().Position
                            root.CFrame = CFrame.new(zonePos + Vector3.new(0, 5, 0))
                            root.AssemblyLinearVelocity = Vector3.zero -- Stop
                            root.AssemblyAngularVelocity = Vector3.zero
                            Library:Notify({ Title = "Zone", Description = "Delivered to Skills Zone!", Time = 2 })
                        end
                        
                        -- Stay at zone for 2 seconds
                        task.wait(2)
                        
                        -- Resume AutoFarm
                        if wasFarming then
                            AutoFarm:Start()
                            Library:Notify({ Title = "Done!!", Description = "Farm resumed!", Time = 2 })
                        end
                    end
                end)
                task.wait(5)
...
-- ==========================================
-- AUTO BUY BASE UPGRADES (HQ Waves Upgrades)
-- ==========================================

getgenv().AutoBuyBaseUpgrades = false

local BaseUpgradeNames = {"Max", "Regen", "Replenish", "Refills", "Revive"}

WavesSettingsGroup:AddToggle("AutoBuyBaseUpgradesToggle", {
    Text = "Buy Base Upgrades",
    Default = false,
    Tooltip = "Auto buys selected HQ base upgrades using Waves gold"
})
    ...
    if getgenv().AutoBuyBaseUpgrades then
        task.spawn(function()
            while getgenv().AutoBuyBaseUpgrades do
                local selected = Options.BaseUpgradesSelectDropdown.Value or {}
                local boughtAny = false

                for _, upgName in ipairs(BaseUpgradeNames) do
                    if not getgenv().AutoBuyBaseUpgrades then break end
                    if selected[upgName] then
                        local ok, result = pcall(function()
                            return getRemote:InvokeServer("Waves", "Upgrade", {upgName})
                        end)
                ...
                task.wait(boughtAny and 1 or 5)
            end
        end)
    end
end)

WavesSettingsGroup:AddDropdown("BaseUpgradesSelectDropdown", {
    Values = BaseUpgradeNames,
    Default = {},
    Multi = true,
    Text = "Select Upgrades",
    Tooltip = "Max / Regen / Replenish / Refills / Revive"
})
...
TSCratesGroup:AddSlider("TSPickupWaitSlider", {
	...
	Max = 8,
	Rounding = 1,
	Tooltip = "Wait x sec after collecting the supply!"
})
...
local function tpToWatchTower(index)
    local wt = getWatchTower(index)
    if not wt then
        Library:Notify({ Title = "❌ Not Found", Description = "WatchTower_" .. index .. " not found!", Time = 3 })
        return
    end
    ...
    if pos then
        ...
        if root then
            ...
            Library:Notify({ Title = "📍 Teleported", Description = "Watch Tower " .. index, Time = 2 })
        end
    end
end

TSWatchTowerGroup:AddLabel("Manual Teleport")

TSWatchTowerGroup:AddButton({
    Text = "TP to Watch Tower 1",
    Func = function() tpToWatchTower(1) end,
    Tooltip = "Teleport to WatchTower_1"
})

TSWatchTowerGroup:AddButton({
    Text = "TP to Watch Tower 2",
    Func = function() tpToWatchTower(2) end,
    Tooltip = "Teleport to WatchTower_2"
})

TSWatchTowerGroup:AddButton({
    Text = "TP to Watch Tower 3",
    Func = function() tpToWatchTower(3) end,
    Tooltip = "Teleport to WatchTower_3"
})
...
Toggles.AutoPrestigeToggle:OnChanged(function()
	...
	if not getgenv().AutoPrestige then return end

	task.spawn(function()
		if game.PlaceId ~= 14916516914 then
			Library:Notify({ Title = "Auto Prestige", Description = "Works in lobby!", Time = 3 })
			getgenv().AutoPrestige = false
		...
		if not slot then
			Library:Notify({ Title = "Auto Prestige", Description = "Slot not selected!", Time = 3 })
			getgenv().AutoPrestige = false
			Toggles.AutoPrestigeToggle:SetValue(false)
			return
		end

		Library:Notify({ Title = "Auto Prestige", Description = "Slot " .. slot .. " starting...", Time = 2 })

		while getgenv().AutoPrestige do
			-- Optional gold gate
			local requiredGold = Options.PrestigeGoldSlider.Value * 1000000
			if requiredGold > 0 then
				local pData = nil
				for i = 1, 5 do
				...
				if not pData or not pData.Slots or not pData.Slots[slot] then
					task.wait(5)
					continue
				end
				local gold = pData.Slots[slot].Currency.Gold
			...
			-- STEP 1: Request Boost draft options
			local ok1, r1a, r1b, r1c = pcall(function()
				return getRemote:InvokeServer("S_Equipment", "Talents")
			end)

			print("[AutoPrestige] Step1 (Boosts):", ok1, r1a, r1b, r1c)

			if not ok1 or r1b == nil then
				Library:Notify({ Title = "Auto Prestige", Description = "Boost draft failed, check F9!", Time = 4 })
				task.wait(5)
				continue
			end

			local boostOptions = r1b
			local chosenBoost = boostOptions[1]

			task.wait(0.5)

			-- STEP 2: Confirm boost -> request Talent draft options
			local ok2, r2a, r2b, r2c = pcall(function()
				return getRemote:InvokeServer("S_Equipment", "Talents")
			end)

			print("[AutoPrestige] Step2 (Talents):", ok2, r2a, r2b, r2c)

			if not ok2 or r2b == nil then
				Library:Notify({ Title = "Auto Prestige", Description = "Talent draft failed, check F9!", Time = 4 })
				task.wait(5)
				continue
			end

			local talentOptions = r2b
			local chosenTalent = talentOptions[1]

			task.wait(0.5)

			-- STEP 3: Final Prestige submit
			local ok3, result = pcall(function()
				return getRemote:InvokeServer("S_Equipment", "Prestige", { Boosts = chosenBoost, Talents = chosenTalent })
			end)

			print("[AutoPrestige] Step3 (Prestige):", ok3, result)

			if ok3 and result ~= nil and result ~= false then
				Library:Notify({ Title = "Successfully Prestiged", Description = "Boost: " .. tostring(chosenBoost) .. " | Talent: " .. tostring(chosenTalent), Time = 5 })
				task.wait(1)
			else
				Library:Notify({ Title = "Auto Prestige", Description = "Prestige submit failed, check F9!", Time = 4 })
				task.wait(5)
			end
		end
	end)
